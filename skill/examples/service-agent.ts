import path from "node:path";
import { fileURLToPath } from "node:url";
import { PactClient } from "@pactnet/agent-sdk";
import dotenv from "dotenv";
import { ethers } from "ethers";

const repoRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../..");
dotenv.config({ path: path.join(repoRoot, ".env") });

async function main() {
  const provider = new ethers.JsonRpcProvider(process.env.PHAROS_RPC_URL!);
  const signer = new ethers.Wallet(process.env.SERVICE_AGENT_PRIVATE_KEY!, provider);
  const pactnet = new PactClient(
    provider,
    signer,
    process.env.PACT_ENGINE_ADDRESS!,
    process.env.NEXT_PUBLIC_ARBITER_URL ?? "http://localhost:3001",
    process.env.REPUTATION_NFT_ADDRESS!
  );

  const customerAgent = process.env.CUSTOMER_AGENT_ADDRESS!;

  const created = await pactnet.createPact({
    agentB: customerAgent,
    commitmentText: "I commit to returning a successful API response for the requested task within 1 hour, or I forfeit my bond.",
    bondWei: ethers.parseEther("0.01").toString(),
    deadlineSeconds: 60 * 60
  });

  await pactnet.submitEvidence(created.pactId, [
    {
      type: "api_response",
      value: "https://api.example.com/jobs/job_123",
      metadata: {
        status: "success",
        jobId: "job_123"
      },
      timestamp: Math.floor(Date.now() / 1000)
    }
  ]);

  const trust = await pactnet.getAgentTrustScore(await signer.getAddress());
  console.log({ pactId: created.pactId, reliabilityPct: trust.reliabilityPct, totalBondHonored: trust.totalBondHonored });
}

main().catch((err) => {
  console.error(err);
  process.exit(1);
});
